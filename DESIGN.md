...
- Light mode is the supported first theme. Use the semantic CSS tokens in `web/src/app/styles.css`; do not hardcode new product colors in components.
...
Desktop uses a 264-pixel dark sidebar and a restrained content column. Keep Overview, Status, Users, Providers, Models, API keys, Requests, Usage, Audit, and Settings in stable order according to role. The signed-in account belongs at the bottom and opens email, password, session, and logout controls.
...
Usage charts use the shadcn Chart component with Recharts v3. Every chart needs an accessible layer, a readable text summary or table, explicit units and periods, and non-color-only series labels.
