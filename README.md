...
Gateway-owned OpenAI Vector Stores support the official SDK's create, list, retrieve, update, and delete lifecycle for empty local stores. Resources belong to the creating key, require `vector_stores:manage`, support bounded metadata and optional expiry, and share retained-storage ceilings. File attachment, chunking, semantic search, and Responses `file_search` remain pending until their ingestion and accounting contract is implemented.
