...
Anthropic Messages also supports bounded native basic web search on the built-in `anthropic` preset. Non-streaming JSON requests may include one direct `web_search_20250305` tool with `max_uses` from 1 through 4, optional domain controls, and approximate user location. The public and upstream models need `chat` and `web_search`, and the key needs `chat:generate` plus `messages:web_search`. Native citations, encrypted result state, `pause_turn`, and reported search calls are preserved. The gateway does not translate or fall back after dispatch; lowest-cost, free-only, and spend policies reject these requests because provider search cost remains unknown.
