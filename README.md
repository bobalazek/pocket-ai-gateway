...
OpenAI Responses supports stateless streaming, local storage, durable background execution, polling, cancellation, input-item listing, deletion, native response compaction on the OpenAI preset, key-owned Conversation resources, and synchronous JSON conversation attachment. Embeddings and token counting use native capable targets so the gateway never invents vector spaces or tokenizer results.
