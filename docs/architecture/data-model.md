...
| attempts | id, request_id, ordinal, target/connection/model snapshots, price_version, state, timings, total/cache/output usage provenance | One upstream dispatch intent; unique request + ordinal |
...
| usage_daily | date + relevant user/key/model/connection dimensions, aggregate total/cache/output counters | Data store; rebuildable summaries, not enforcement source |
