...
| media_jobs | creating user/key, request/attempt, model/connection/provider IDs, encrypted input/output, provider job ID, state/cancellation/poll retry timestamps | Durable Replicate, Together video, or custom scripted asynchronous job; bounded retry state and `interrupted_unknown` preserve honest lifecycle recovery, input is cleared at terminal state, output is visible only to the creating key, and admin reads are redacted |
