...
| openai_batch_items | Batch ID, ordinal, unique custom ID, bounded request/result JSON, execution state, request/attempt IDs, lease timestamps | Parent Batch and creating key; 1–4 non-streaming Responses requests per input File; each executes once through ordinary Responses admission/accounting |
