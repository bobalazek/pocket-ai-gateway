...
| openai_vector_stores | gateway Vector Store ID, creating key/user, bounded name/description/metadata, activity and optional expiry policy/timestamps | Creating key; metadata-only lifecycle; shared retained-resource ceilings; expiry or explicit deletion removes the store; file ingestion/search remain separate |
