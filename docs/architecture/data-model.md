...
| openai_batch_items | Batch ID, ordinal, unique custom ID, bounded request/result JSON, execution state, request/attempt IDs, lease timestamps | Parent Batch and creating key; 1–4 non-streaming Responses, Chat Completions, Embeddings, or Moderations requests per input File; each executes once through ordinary endpoint admission/accounting; moderation results leave Batch aggregate usage null because the response has no usage object |
| openai_files | gateway File ID, creating key/user, filename, purpose, encrypted bytes/nonce, created/expiry timestamps | Creating key; processed immediately; input uses batch, generated Batch output uses batch_output, and explicit deletion or expiry removes content |
