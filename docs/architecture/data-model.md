...
| provider_adapter_scripts | connection ID, request/response JavaScript, updated timestamp | Custom OpenAI-compatible connection; trusted administrator-managed synchronous JSON transforms with time and I/O limits, included in portable configuration |
...
| attempts | id, request_id, ordinal, target/connection/model snapshots, price_version, state, timings, total/cache/output usage provenance, nullable web-search call ceiling/count and web-fetch presence | One upstream dispatch intent; unique request + ordinal; versioned per-call search fees are added to token cost when complete pricing and reported usage exist. Combined fetch and unknown cache-creation costs remain unpriced; retained non-content counters support safe historical repricing |
| media_jobs | creating user/key, request/attempt, model/connection/provider IDs, encrypted input/output, provider job ID, state/cancellation/poll retry timestamps | Durable Replicate, Together video, Gemini Veo, or custom scripted asynchronous job; bounded retry state and `interrupted_unknown` preserve honest lifecycle recovery, input is cleared at terminal state, output is visible only to the creating key, and admin reads are redacted |
...
| message_batches | gateway batch ID, creating key/user, processing/cancellation state, created/ended/retention-expiry timestamps | Creating key; in_progress → canceling → ended; stored expires_at is the 29-day whole-resource retention deadline, while the public 24-hour processing deadline is derived from created_at |
| message_batch_items | batch ID, ordinal, unique custom ID, bounded params/result JSON, execution state, request/attempt IDs, lease timestamps | Parent batch and creating key; 1–4 items per 16 MiB submission; each item executes once through the ordinary Messages admission/accounting path and may finish independently |
| openai_uploads | gateway Upload ID, creating key/user, filename, purpose/MIME type, declared bytes, status, completed File ID, lifecycle timestamps | Creating key; pending for one hour, then completed or cancelled; pending reservations share retained-resource capacity |
| openai_upload_parts | gateway Part ID, Upload ID, encrypted bytes/nonce, plaintext size, creation timestamp | Parent Upload and creating key; 1–16 non-empty Parts; cascades on Upload deletion and is removed on completion or cancellation |
| openai_batches | gateway Batch ID, creating key/user, input File, endpoint/window, metadata, processing/cancellation state, request counts, output/error File IDs, lifecycle timestamps | Creating key; in_progress → finalizing → completed, or expired/cancelling/cancelled; unfinished work expires after 24 hours and Batch metadata expires 30 days after creation |
| openai_batch_items | Batch ID, ordinal, unique custom ID, bounded request/result JSON, execution state, request/attempt IDs, lease timestamps | Parent Batch and creating key; 1–4 non-streaming Responses, Chat Completions, Embeddings, Moderations, image-generation, or image-edit requests per input File; each executes once through ordinary endpoint admission/accounting; aggregate usage is available only when terminal Batch usage is complete, otherwise it is null |
| openai_files | gateway File ID, creating key/user, filename, client purpose, encrypted bytes/nonce, created/expiry timestamps | Creating key; processed immediately; supported OpenAI input purposes remain local, generated Batch output uses batch_output, and explicit deletion or expiry removes content |
| openai_vector_stores | gateway Vector Store ID, creating key/user, bounded name/description/metadata, activity and optional expiry policy/timestamps | Creating key; shared retained-resource ceilings; expiry or explicit deletion removes the store and its attachments |
| openai_vector_store_files | Vector Store ID, same-key File ID, bounded scalar attributes, chunking marker, creation timestamp | Parent Vector Store; attachment metadata shares retained-resource ceilings; source File content remains independently owned and encrypted; parsed content/search remain separate |
