...
For Replicate, create one connection for an account credential and its `https://api.replicate.com/v1` endpoint, then add as many upstream models as needed. An upstream ID can be `owner/model`, `version:<version-id>`, or `deployment:owner/name`; publish each with the `media_jobs` capability and a fixed route to its upstream ID. `POST /api/v1/media/jobs` accepts that model's provider-defined JSON `input` and returns a gateway job ID for polling. The gateway does not invent a universal input form or validate each model's changing schema: consult the selected Replicate model's input documentation. Use another connection only when the credential or endpoint differs. Model-specific JavaScript transforms are not available on the Replicate preset; custom scripts belong to a separate custom connection.
