...
### Request timing and more System One servers — 2026-09-25

Requests now record streaming or synchronous mode (server-sent events and Realtime/Live sessions are streaming), and attempts record when the first response byte reached the gateway (migration 0040). Request history reports duration and time to first byte per request and attempt, and filters by mode. Usage breakdowns add a `mode` dimension, average/p95 time to first byte, and a `p95_first_byte` sort, shown in the dashboard by model, operation, and response mode. Asynchronous work (background Responses, batches, media jobs) has no first-byte time. New System One presets cover Vercel AI Gateway (Jev), Codiv, Kev, CLM, and OpenJev, and upstream errors from each server's format are normalized. Tests cover mode and first-byte aggregation, history fields, the mode filter, recording on real streaming and synchronous gateway requests, and all five error shapes. The quick gate (including 87 frontend tests) and targeted race tests for usage, gateway, and storage passed. No live provider was called.

### Node.js 26 and Ubuntu 26.04 toolchain — 2026-09-25

The dashboard toolchain moved from Node.js 22 to Node.js 26 (v26.10.0; LTS from October 28, 2026). Workflow actions use their Node 24 majors, and the verify and release jobs run on `ubuntu-26.04`. The container build installs pnpm 10.30.3 with npm because Node.js 26 no longer bundles Corepack; this was found when the first Node 26 check failed. In a `node:26-bookworm-slim` container, the dashboard's typecheck, production static export, and 39 frontend tests passed. The 48 official-SDK tests were not run there because the container has no Go; GitHub Verify runs them. The Docker Compose backup/restore/restart E2E and an arm64 production image build passed with the new Dockerfile.

### Full-page screenshot gallery — 2026-09-25

The published overview showed the desktop account menu floating above an empty sidebar strip. Full-page captures kept the 900-pixel viewport while imaging the taller document, so the sticky, viewport-height sidebar ended at the fold. The capture script now resizes the viewport to the document height before each capture. Every image is a complete page; the 12 section close-ups and 5 viewport previews were removed. All 24 images (19 desktop, 5 mobile) were regenerated from scratch. A pixel check confirmed the account avatar sits 19 pixels from the bottom of every desktop page; the same check flagged the old overview at 132 pixels.

Reviewing the new analytics page exposed a demo-seed bug: backdated requests kept their real first-byte time, so the example showed a six-day time to first byte. The seed now shifts `first_byte_at` with the other timestamps and streams every other OpenAI request, so both response modes appear. `go test ./scripts/demo` now asserts that streaming and synchronous requests exist and that every first byte falls within its request. This assertion failed on the old seed with 93 out-of-range first bytes.

### Dashboard controls — 2026-09-25

The full-page Providers capture showed native checkboxes for on/off settings, native select arrows pressed against the right edge, a raw `custom · … · enabled` line, and three example connections sharing one base URL. A `Switch` primitive (native checkbox with `role="switch"`) now covers private-network access, connection enablement, route-preview streaming, free-only routing, scheduled catalog refresh, and playground streaming. Selects draw a padded chevron, read-only fields render muted, and help-text links are distinguishable. Connection cards show preset, adapter, and credential labels, a compact base URL, and an enabled switch. The preset form shows the adapter label rather than the preset name and lists readable capabilities. Demo connections use distinct paths, and the seed test asserts they differ. Typecheck and the 87 frontend tests passed, the design detector found nothing, and all 24 full-page screenshots were regenerated with every desktop account menu anchored.
