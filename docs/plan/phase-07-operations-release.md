...
- The local release gate passed Go tests and race checks, 14 frontend/SDK files with 72 tests, frontend production build, OpenAPI validation, Linux amd64/arm64 builds and execution, embedded-binary smoke, encrypted backup/restore, checksum verification, a multi-platform image build, and the disposable Compose recovery journey. Live cloud certification and publication credentials remain explicit maintainer tasks.
