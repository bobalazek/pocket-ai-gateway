...
Status: in progress. The broader provider preset catalog, deterministic metadata checks, gateway-owned stored/background Response lifecycle, gateway-owned stored Chat Completion lifecycle, bounded gateway-owned Anthropic Message Batches, bounded native OpenAI Responses web search, bounded native Anthropic JSON/SSE basic web search, and native OpenAI image edit/variation and audio transcription/translation operations are implemented; live certification and larger resource/auth work remain open. [Plan index](README.md)
...
| Full API parity inventory | Official OpenAI/Anthropic/Gemini endpoints and fields are inventoried; gateway-owned Response, stored Chat Completion, Conversation, and bounded Anthropic Message Batch lifecycles, compaction, input-token counting, synchronous/background/buffered-stream attachment, bounded native OpenAI Responses web search, bounded native Anthropic JSON/SSE basic web search, native OpenAI Moderations, bounded non-streaming image generation/edit/variation, buffered OpenAI speech, multipart OpenAI transcription/translation, and native Anthropic Messages prompt caching are implemented; add files, OpenAI/Gemini and provider-owned batches, other cache APIs, remaining hosted tools, remaining multimedia, and realtime surfaces with explicit capability matrix | Full compatibility goal; separate per-operation contracts and evidence; never equate unsupported with done |
