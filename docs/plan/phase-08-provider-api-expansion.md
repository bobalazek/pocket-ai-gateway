...
Status: in progress. The broader provider preset catalog, deterministic metadata checks, and gateway-owned synchronous Response retrieval/deletion are implemented; background execution, live certification, and larger resource/auth work remain open. [Plan index](README.md)
...
| Full API parity inventory | Official OpenAI/Anthropic/Gemini endpoints and fields are inventoried; gateway-owned synchronous Response retrieval/deletion is implemented; add remaining input items/conversations, files/batches/caches, hosted tools, multimedia/realtime surfaces with explicit capability matrix | Full compatibility goal; separate per-operation contracts and evidence; never equate unsupported with done |
| Background delivery | Durable job submission, polling, cancellation, result retention, creating-key ownership and charging contract | Accepted in ADR-015; reuse the stored Response resource and shared enforcement path |
