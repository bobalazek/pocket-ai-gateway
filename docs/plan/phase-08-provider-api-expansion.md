...
| Full API parity inventory | Official OpenAI/Anthropic/Gemini endpoints and fields are inventoried; gateway-owned Response retrieval/deletion/cancellation/input items are implemented; add conversations, files/batches/caches, hosted tools, multimedia/realtime surfaces with explicit capability matrix | Full compatibility goal; separate per-operation contracts and evidence; never equate unsupported with done |
