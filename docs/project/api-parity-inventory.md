...
| Hosted web search | Constrained, native Anthropic preset only | One basic `web_search_20250305` or dynamic-filtering `web_search_20260209` plus ordinary application tools; JSON or SSE; required 1–4 `max_uses`; version-aware direct/code-execution callers and boolean `strict`; optional mutually exclusive domain controls and approximate location; public/upstream `chat` and `web_search`, plus `web_search_dynamic` for code execution; `chat:generate` plus `messages:web_search`; native result/citation/continuation/error/caller shapes, public-model normalization in `message_start`, cumulative terminal search/code-execution usage, and reported calls; no deferred loading, prompt-cache combination, 20260318 response inclusion, translation, post-dispatch fallback, free-only/lowest-cost/spend policies, or known search cost |
| Hosted web fetch | Constrained, native Anthropic preset only | One basic `web_fetch_20250910` or dynamic-filtering `web_fetch_20260209`; JSON or SSE; required 1–4 `max_uses` and approximate 1–16,384 text `max_content_tokens` that does not bound binary PDFs; version-aware direct/code-execution callers and boolean `strict`; optional citations and mutually exclusive ten-host domain controls; public/upstream `chat` and `web_fetch`, plus `web_fetch_dynamic` for code execution; `chat:generate` plus `messages:web_fetch`; native text/PDF result, caller and error blocks, public model, terminal token/fetch/code-execution usage; provider-reported token pricing; no deferred loading, token/spend policies, free-only/lowest-cost routing, prompt-cache/search combination, Message Batches, 20260309 cache bypass, 20260318 response inclusion, translation, or post-dispatch retry |
