...
| Vector Stores | Constrained, gateway-owned | Five store lifecycle and five file-subresource SDK methods; same-key File attachment, bounded scalar attributes, aggregate active bytes/counts, `vector_stores:manage`, 1–365-day optional `last_active_at` expiry, shared retained-resource ceilings, validated keyset pagination, cleanup, and uniform not-found boundary; create-with-files, static chunking, file batches, parsed content, semantic search, and Responses `file_search` remain pending |
