...
Cross-provider generation covers the shared documented subset. Anthropic client streaming remains native-target only because input usage is required in the first Anthropic event. Native Anthropic Messages supports bounded ephemeral prompt-cache controls with explicit capability and accounting gates; cache directives are not translated. Responses web search requires an OpenAI-adapter target using the built-in `openai` preset, explicit call/output ceilings, public/upstream capability, and a separate key scope; its search-call cost remains unknown. Embeddings, moderations, and token counting require native capable targets. Other hosted tools, opaque reasoning blocks, files, batches, realtime, other cache APIs, and provider-owned conversations are unsupported until explicitly implemented and tested.
