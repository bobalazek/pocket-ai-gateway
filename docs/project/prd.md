...
Scopes: chat:generate, completions:generate, messages:batches, messages:web_search, messages:web_fetch, responses:generate, responses:web_search, embeddings:generate, moderations:classify, images:generate, images:edit, images:variation, audio:speech, audio:transcribe, audio:translate, batches:manage, files:manage, vector_stores:manage, models:read, tokens:count. Effective access intersects user grants, key grants, public-model publication, allowed connections, capability support, and all limits. Empty grants deny; unrestricted access requires an explicit administrator-controlled mode.
...
Expose OpenAI Chat Completions/Responses plus native legacy Completions with bounded gateway-owned Batches for Responses, Chat Completions, legacy Completions, Embeddings, Moderations, Image Generations, and Image Edits, native web search, gateway-owned file search, Responses input-token counting, Files, Vector Store and file-attachment lifecycles, embeddings, moderations, bounded JSON/SSE image generation, GPT Image edits, DALL-E 2 variations, buffered speech, and models under /api/openai/v1/; Anthropic Messages with bounded native basic web search, basic web fetch, count_tokens/models, and gateway-owned Message Batches under /api/anthropic/v1/; and native Gemini Interactions/generateContent/streamGenerateContent/countTokens/embedContent/batchEmbedContents/models under /api/gemini/v1beta/. DALL-E 2 edits remain unsupported until their distinct multipart contract is implemented. The path selects the protocol; headers never switch dialects. Global aliases are disabled by default. Preserve each dialect's response/error/event shape without a gateway JSON envelope.
...
Legacy `POST /api/openai/v1/completions` accepts the official JSON and SSE request shapes only on OpenAI or OpenAI-compatible targets whose public and upstream models publish `completions`; the key requires `completions:generate`. The request body, JSON response, and each SSE event are limited to 16 MiB. The gateway validates prompt, candidate, sampling, stop, log-probability, streaming, suffix, and user fields before dispatch; multiplies the per-candidate output reservation by prompt cardinality and the greater of `n` and `best_of`; preserves native choices, usage, and `[DONE]`; and normalizes the public model alias in JSON and stream events. A request without positive `max_tokens` cannot satisfy token, output-token, or spend enforcement. Legacy prompt semantics are never translated to another protocol.
...
Anthropic Messages hosted web tools accept one basic or dynamic-filtering search/fetch tool in JSON or SSE requests: `web_search_20250305`/`web_search_20260209`/`web_search_20260318` or `web_fetch_20250910`/`web_fetch_20260209`/`web_fetch_20260309`/`web_fetch_20260318`. One search tool and one fetch tool may be combined. Each requires `max_uses` from 1 through 4, matching public/upstream base capability, the built-in `anthropic` preset, and both `chat:generate` and its feature scope. Code-execution invocation additionally requires the matching `web_search_dynamic` or `web_fetch_dynamic` capability; `response_inclusion` additionally requires `web_search_response_inclusion`/`web_fetch_response_inclusion`, and fetch `use_cache` additionally requires `web_fetch_cache_bypass`. A combined request requires both scopes and both capability sets. Dynamic versions default to Anthropic-managed code execution and may be forced direct through `allowed_callers`; optional `strict` is boolean. Prompt caching may be combined, including a `cache_control` breakpoint on the tool definition, and cache counters are preserved alongside web-tool usage. Native server-tool/result/caller blocks, encrypted continuation state, citations, `pause_turn`, embedded tool errors, SSE event shapes, cumulative terminal token/tool/code-execution usage, and reported calls are preserved. The gateway rejects deferred loading, translation, post-dispatch fallback, response-inclusion or cache-bypass on earlier versions, and unsafe routing/spend policies before dispatch.

Anthropic Messages basic web fetch accepts one `web_fetch_20250910` tool in JSON or SSE requests. It requires `max_uses` from 1 through 4, an approximate text `max_content_tokens` limit from 1 through 16,384, public/upstream `chat` and `web_fetch`, the built-in `anthropic` preset, and both `chat:generate` and `messages:web_fetch`. Up to ten plain hostnames may be allowed or blocked, never both. Native fetched text/PDF document blocks, citations, embedded fetch errors, SSE events, token usage, and reported fetch-call usage are preserved. Binary PDF input is outside the text-token bound, so token/spend policies, free-only routing, and lowest-cost routing reject fetch requests before dispatch. The gateway also rejects search, prompt-cache, translation, local Message Batch, and post-dispatch fallback combinations. Provider-reported usage uses ordinary versioned token pricing.

Gateway-owned Anthropic Message Batches expose create, retrieve, list, cancel, delete, and JSONL results for the creating key. A 16 MiB submission contains one to four uniquely identified ordinary non-streaming Messages and requires `chat:generate` plus `messages:batches`. Items use the creating key's current grants and the shared local Messages routing, admission, and accounting path. `expires_at` is a 24-hour processing deadline; 29 days after creation the whole batch and its results are deleted. The gateway does not claim Anthropic's provider-owned 100,000-item ceiling or native 50% batch price.

Gateway-owned OpenAI Batches expose create, retrieve, list, and cancel for the creating key. Each Batch references an unexpired same-key `purpose=batch` File, uploaded with `files:manage`, containing one to four uniquely identified non-streaming `/v1/responses`, `/v1/chat/completions`, `/v1/completions`, `/v1/embeddings`, `/v1/moderations`, `/v1/images/generations`, or `/v1/images/edits` requests for one public model. Batch creation requires `batches:manage` plus the selected endpoint scope, including `completions:generate` for legacy Completions, `images:generate` for Image Generations, and `images:edit` for Image Edits. Items use the creating key's current grants and the ordinary local endpoint routing, admission, accounting, and pricing path. Legacy Completions reuse their strict direct validator and native target contract. Embeddings use one vector-space target. Moderations and Image Generations reuse direct validation; Images use native routing without fallback. Batch Image Edits use JSON-reference validation, accept HTTPS or bounded PNG/JPEG/WebP data-URL references, and reject provider `file_id` references. Endpoint output remains in its ordinary shape; the Batch resource carries the public model alias. Aggregate usage is available only when terminal Batch usage is complete; otherwise it is null. Success and failure JSONL are separate same-key `purpose=batch_output` Files. Processing expires after 24 hours; generated output files expire after 30 days by default or an earlier requested 1-hour through 30-day interval. The gateway does not claim OpenAI's provider-owned 50,000-request/200 MB ceilings, separate rate pool, unbounded output, or 50% discount.
...
Responses supports native and translated POST /api/openai/v1/responses across the three provider families, including typed output items, function tools/results, and stream lifecycle events. Non-streaming responses default to gateway-owned storage under the creating API key; `store:false` disables the stored Response resource. Durable background execution uses the same ownership contract. Gateway-owned Conversations attach to synchronous, background, and bounded buffered-stream Responses as separate retained state; failed terminal streams leave them unchanged. Bounded web search accepts one native OpenAI `web_search` alongside function tools only on an OpenAI-adapter target using the built-in `openai` preset with public/upstream `chat` and `web_search` capabilities plus `responses:web_search`; it requires 1–4 `max_tool_calls` and positive `max_output_tokens`, records token usage and search calls separately, and includes a configured effective-dated per-call fee in cost when available. Web search rejects streaming, Conversations/provider references, preview/image search, unlimited search-token return, translation, post-dispatch fallback and free-only/lowest-cost routing before dispatch. Spend enforcement requires a configured per-call fee, an explicit cache-read rate, and bounded token estimates. Bounded gateway-owned `file_search` searches 1–10 same-key local Vector Stores through a native OpenAI/OpenAI-compatible Responses target, requires `responses:file_search`, emits official tool output, and aggregates every provider turn's usage for stateless, stored, or background JSON. It rejects streaming, Conversations/provider references, web-search combinations, semantic/hybrid rankers, prompt templates/cache controls, translation, and fallback. Provider-owned response or conversation chains and other hosted tools require separate phase 8 contracts. Opaque provider-affine inputs require a matching route or explicit rejection.
