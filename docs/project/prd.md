...
Expose OpenAI Chat Completions/Responses/embeddings/moderations/models under /api/openai/v1/, Anthropic Messages/count_tokens/models under /api/anthropic/v1/, and native Gemini generateContent/streamGenerateContent/countTokens/embedContent/batchEmbedContents/models under /api/gemini/v1beta/. The path selects the protocol; headers never switch dialects. Global aliases are disabled by default. Preserve each dialect's response/error/event shape without a gateway JSON envelope.
