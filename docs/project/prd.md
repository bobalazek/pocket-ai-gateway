...
Anthropic Messages basic web search accepts one direct `web_search_20250305` tool alongside ordinary application tools in JSON requests. It requires `max_uses` from 1 through 4, public/upstream `chat` and `web_search`, the built-in `anthropic` preset, and both `chat:generate` and `messages:web_search`. Native server-tool/result blocks, encrypted continuation state, citations, `pause_turn`, embedded search errors, and reported search-call usage are preserved. The gateway rejects streaming, prompt-cache combinations, newer dynamic-filtering versions, translation, post-dispatch fallback, free-only/lowest-cost routing, and spend policies before dispatch. Search calls are separate from tokens and their cost remains unknown.
