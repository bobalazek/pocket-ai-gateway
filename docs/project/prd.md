...
Full fidelity of the three client protocols is the product goal. v0.1 covers the generation, tool, stream, counting, and embedding operations specified below; provider-specific features require tested mappings or explicit unsupported-capability errors. Native Anthropic Messages prompt-cache controls and bounded basic web search, bounded gateway-owned Anthropic Message Batches, bounded gateway-owned OpenAI Responses, Chat Completions, Embeddings, Moderations, Image Generations, and Image Edits Batches, and bounded native OpenAI Responses web search are implemented. Phase 8 contains the remaining API-parity work: broader state/resource ownership, general uploads and vector stores, remaining OpenAI/Gemini and provider-owned batch operations, provider-owned cache resources and other caching surfaces, remaining hosted tools, multimedia/realtime, and related provider-specific surfaces. Do not advertise whole-API parity until that inventory is complete and tested.
...
Scopes: chat:generate, messages:batches, messages:web_search, responses:generate, responses:web_search, embeddings:generate, moderations:classify, images:generate, images:edit, images:variation, audio:speech, audio:transcribe, audio:translate, batches:manage, files:manage, models:read, tokens:count. Effective access intersects user grants, key grants, public-model publication, allowed connections, capability support, and all limits. Empty grants deny; unrestricted access requires an explicit administrator-controlled mode.
...
Gateway-owned OpenAI Batches expose create, retrieve, list, and cancel for the creating key. Each Batch references an unexpired same-key `purpose=batch` File, uploaded with `files:manage`, containing one to four uniquely identified non-streaming `/v1/responses`, `/v1/chat/completions`, `/v1/embeddings`, `/v1/moderations`, `/v1/images/generations`, or `/v1/images/edits` requests for one public model. Batch creation requires `batches:manage` plus the selected endpoint scope, including `images:generate` for Image Generations and `images:edit` for Image Edits. Items use the creating key's current grants and the ordinary local endpoint routing, admission, accounting, and pricing path. Embeddings use one vector-space target. Moderations and Image Generations reuse direct validation; Images use native routing without fallback. Batch Image Edits use JSON-reference validation, accept HTTPS or bounded PNG/JPEG/WebP data-URL references, and reject provider `file_id` references. Image output remains an ordinary Images response; the Batch resource carries the public model alias. Aggregate usage is available only when terminal Batch usage is complete; otherwise it is null. Success and failure JSONL are separate same-key `purpose=batch_output` Files. Processing expires after 24 hours; generated output files expire after 30 days by default or an earlier requested 1-hour through 30-day interval. The gateway does not claim OpenAI's provider-owned 50,000-request/200 MB ceilings, separate rate pool, unbounded output, or 50% discount.
