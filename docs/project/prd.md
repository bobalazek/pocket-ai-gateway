...
Anthropic Messages hosted web tools accept one basic or dynamic-filtering search/fetch tool in JSON or SSE requests: `web_search_20250305`/`web_search_20260209` or `web_fetch_20250910`/`web_fetch_20260209`. Each requires `max_uses` from 1 through 4, matching public/upstream base capability, the built-in `anthropic` preset, and both `chat:generate` and its feature scope. Code-execution invocation additionally requires the matching `web_search_dynamic` or `web_fetch_dynamic` capability. Dynamic versions default to Anthropic-managed code execution and may be forced direct through `allowed_callers`; optional `strict` is boolean. Native server-tool/result/caller blocks, encrypted continuation state, citations, `pause_turn`, embedded tool errors, SSE event shapes, cumulative terminal token/tool/code-execution usage, and reported calls are preserved. The gateway rejects deferred loading, prompt-cache combinations, translation, post-dispatch fallback, unsupported later versions, and unsafe routing/spend policies before dispatch.
