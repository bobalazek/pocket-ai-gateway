...
Full fidelity of the three client protocols is the product goal. v0.1 covers the generation, tool, stream, counting, and embedding operations specified below; provider-specific features require tested mappings or explicit unsupported-capability errors. Native Anthropic Messages prompt-cache controls and bounded basic web search, bounded gateway-owned Anthropic Message Batches, bounded gateway-owned OpenAI Responses, Chat Completions, Embeddings, and Moderations Batches, and bounded native OpenAI Responses web search are implemented. Phase 8 contains the remaining API-parity work: broader state/resource ownership, general uploads and vector stores, remaining OpenAI/Gemini and provider-owned batch operations, provider-owned cache resources and other caching surfaces, remaining hosted tools, multimedia/realtime, and related provider-specific surfaces. Do not advertise whole-API parity until that inventory is complete and tested.
...
Gateway-owned OpenAI Batches expose create, retrieve, list, and cancel for the creating key. Each Batch references an unexpired same-key `purpose=batch` File, uploaded with `files:manage`, containing one to four uniquely identified non-streaming `/v1/responses`, `/v1/chat/completions`, `/v1/embeddings`, or `/v1/moderations` requests for one public model. Batch creation requires `batches:manage` plus the selected endpoint scope, including `moderations:classify` for Moderations. Items use the creating key's current grants and the ordinary local endpoint routing, admission, accounting, and pricing path. Embeddings use the public model's single vector-space target without fallback and normalize provider-reported prompt usage into Batch input usage. Moderations reuse ordinary request validation and native routing without fallback; the Batch aggregate usage is null because the response has no usage object. Success and failure JSONL are separate same-key `purpose=batch_output` Files. Processing expires after 24 hours; generated output files expire after 30 days by default or an earlier requested 1-hour through 30-day interval. The gateway does not claim OpenAI's provider-owned 50,000-request/200 MB ceilings, separate rate pool, unbounded output, or 50% discount.
