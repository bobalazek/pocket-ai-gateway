...
`GET /api/v1/usage/breakdown` accepts `dimension=key|user|model|connection|dialect|operation|state` and `sort=requests|known_cost|tokens|p95_latency`, plus the same time, user, key, model, and connection filters as usage summary. It returns request counts, attempt and failed-attempt counts, token/cache/search/tool counters, known spend, unknown attempts, and nullable average/p95 whole-gateway request duration per group. Request counts and duration use request start time; attempt usage and cost use attempt start time, matching the summary. Request counts are distinct across retries. A request that falls back across providers can appear under multiple connections, so provider request totals are not additive. Failed attempts can occur even when the final request succeeds. Known spend excludes attempts without a resolved cost and is not a provider invoice. The endpoint uses offset pagination (default 20, maximum 100 rows per page; maximum offset 10,000), separate from request history's cursor pagination. Retained aggregate history can outlive inspectable request detail.
