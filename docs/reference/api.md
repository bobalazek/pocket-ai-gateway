...
| POST/GET /api/openai/v1/vector_stores | Create an empty key-owned Vector Store or list stores with `vector_stores:manage`, shared capacity, optional expiry, and validated order/after/before pagination |
| GET/POST/DELETE /api/openai/v1/vector_stores/{vector_store_id} | Retrieve, update bounded metadata/expiry, or permanently delete a same-key Vector Store; file ingestion and search remain unsupported |
...
Gateway-owned OpenAI Vector Stores expose create, list, retrieve, update, and delete for empty local resources. They require `vector_stores:manage`, belong to the creating key, share retained-resource ceilings, support bounded metadata and optional 1–365-day expiry anchored at `last_active_at`, and use validated `after`/`before` keyset pagination. Non-empty file attachment, chunking, file batches, parsed content, semantic search, and Responses `file_search` are rejected until the separate ingestion contract exists.
