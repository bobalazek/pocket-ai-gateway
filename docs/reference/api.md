...
| POST/GET /api/openai/v1/vector_stores | Create a key-owned Vector Store or list stores with `vector_stores:manage`, shared capacity, optional expiry, and validated order/after/before pagination |
...
Gateway-owned OpenAI Vector Stores expose create, list, retrieve, update, delete, backend text search, plus attach, list, retrieve, update, detach, parsed content, and atomic file batches for same-key gateway Files. They require `vector_stores:manage`, belong to the creating key, share retained-resource ceilings, support bounded metadata/attributes and optional 1–365-day expiry anchored at `last_active_at`, report active attachment bytes/counts, and use validated `after`/`before` keyset pagination. File batches accept 1–2,000 unique `file_ids` with shared attributes/auto chunking or per-file objects, roll back every attachment if any input fails, and complete in the creation transaction; retrieval, cancellation, and paginated file listing match the pinned SDK while cancellation returns the already-terminal batch. Parsed content accepts UTF-8 text without NUL bytes, removes an optional byte-order mark, derives lossless chunks of at most 64 KiB on request, and stores no duplicate plaintext. Search ranks those chunks by deterministic term-frequency cosine similarity, applies nested attribute filters on the server, supports explicit `ranker: none`, and rejects query rewriting or embedding-backed rankers. Create-with-`file_ids`, static token chunking, binary document parsing, embedding-backed semantic search, and Responses `file_search` remain unsupported.
