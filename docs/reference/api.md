...
| POST /api/openai/v1/vector_stores/{vector_store_id}/search | Backend-only lexical search over parsed UTF-8/ASCII, BOM-marked UTF-16, HTML visible text, DOCX body text, or PPTX slide text with bounded query arrays, nested attribute filters, deterministic 0–1 scores, and at most 50 results |
...
| GET /api/openai/v1/vector_stores/{vector_store_id}/files/{file_id}/content | Return the same-key attachment's decrypted UTF-8/ASCII, BOM-marked UTF-16, bounded HTML visible text, DOCX body text, or PPTX slide text as uncached chunks of at most 64 KiB |
