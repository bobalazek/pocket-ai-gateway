...
| POST /api/v1/media/jobs/{id}/cancel | Idempotently cancel a queued or active owned job. Replicate receives its native cancel request; Together cancellation is local because its video API has no documented cancel operation; a custom adapter receives a normalized cancel action. |
