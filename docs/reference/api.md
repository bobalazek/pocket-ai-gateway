...
| POST /api/openai/v1/batches | Create a key-owned 24-hour Batch from a same-key File containing 1–4 non-streaming `/v1/responses`, `/v1/chat/completions`, `/v1/embeddings`, `/v1/moderations`, or `/v1/images/generations` requests; requires `batches:manage` plus the matching endpoint scope |
