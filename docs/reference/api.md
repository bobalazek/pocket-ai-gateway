...
| POST /api/openai/v1/responses | OpenAI input/output items, function tools/results, typed response lifecycle stream, durable background submission, bounded native web search, or same-key local Vector Store file search |
...
| POST/GET /api/openai/v1/vector_stores | Atomically create a key-owned Vector Store with optional same-key `file_ids`, or list stores with `vector_stores:manage`, shared capacity, optional expiry, and validated order/after/before pagination |
| GET/POST/DELETE /api/openai/v1/vector_stores/{vector_store_id} | Retrieve, update bounded metadata/expiry, or permanently delete a same-key Vector Store |
| POST /api/openai/v1/vector_stores/{vector_store_id}/search | Backend-only lexical search over parsed UTF-8/ASCII, BOM-marked UTF-16, HTML visible text, DOCX body text, PPTX slide text, or XLSX worksheet values with bounded query arrays, nested attribute filters, deterministic 0–1 scores, and at most 50 results |
| POST/GET /api/openai/v1/vector_stores/{vector_store_id}/files | Attach an unexpired same-key File with bounded scalar attributes, or list active attachments with validated pagination |
| GET/POST/DELETE /api/openai/v1/vector_stores/{vector_store_id}/files/{file_id} | Retrieve, update attributes, or detach a same-key Vector Store File; detaching preserves the source File |
| GET /api/openai/v1/vector_stores/{vector_store_id}/files/{file_id}/content | Return the same-key attachment's decrypted UTF-8/ASCII, BOM-marked UTF-16, bounded HTML visible text, DOCX body text, PPTX slide text, or XLSX worksheet values as uncached chunks of at most 64 KiB |
| POST /api/openai/v1/vector_stores/{vector_store_id}/file_batches | Atomically attach 1–2,000 same-key Files using shared or per-file attributes and auto chunking |
| GET/POST /api/openai/v1/vector_stores/{vector_store_id}/file_batches/{batch_id}[/cancel] | Retrieve a completed local batch or idempotently request cancellation after its atomic completion |
| GET /api/openai/v1/vector_stores/{vector_store_id}/file_batches/{batch_id}/files | List the batch's active attachments with validated status/order/cursor pagination |
...
| POST /api/openai/v1/audio/speech | Native text-to-speech with buffered output up to 16 MiB or explicit HTTP streaming up to 64 MiB; built-in, custom-reference, and provider voice names; raw audio or SSE passthrough; explicit public model and `audio:speech`; no post-dispatch fallback, strict token/spend/free-only policies, or lowest-cost routing |
...
| POST /api/v1/media/jobs | Create a persistent asynchronous image, video, audio, or provider-defined job with `media:generate`; the body is `{model, media_type, input}` and returns `202` with `{job}` plus `X-Pocket-AI-Request-ID`. Replicate predictions, Together video jobs, Gemini Veo long-running operations, and custom scripted adapters are supported. Raw input is encrypted until dispatch finishes and is never returned. |
| GET /api/v1/media/jobs and GET /api/v1/media/jobs/{id} | An inference key sees only its own jobs; owner/admin browser sessions can inspect all safe job metadata. Only the creating inference key can retrieve decrypted terminal output. |
| POST /api/v1/media/jobs/{id}/cancel | Idempotently cancel a queued or active owned job. Replicate receives its native cancel request; Together and Gemini cancellation stop local polling because their reviewed video contracts have no documented cancel operation; a custom adapter receives a normalized cancel action. |
| GET /api/openai/v1/realtime?model={public_model} | Native [OpenAI Realtime WebSocket](https://developers.openai.com/api/docs/guides/voice-websockets) proxy with `realtime:connect`, explicit public/upstream `realtime`, 16 MiB per-frame and 10-minute session limits, pre-upgrade routing/admission, and terminal token accounting when `response.done` reports usage; HTTP/1.1 WebSocket only, with no WebRTC, SIP, Live-session resource API, or cross-provider translation |
| GET /api/openai/v1/live | Native [OpenAI primary Live WebSocket](https://developers.openai.com/api/reference/typescript/resources/live) proxy. Send `session.start` first with the public model; the gateway rewrites it and relays bidirectional audio, text, and delegation events under `realtime:connect`. WebRTC/SIP and stored-session controls are intentionally absent. |
| GET /api/gemini/v1beta/live | Native [Gemini BidiGenerateContent WebSocket](https://ai.google.dev/api/live) proxy. Send `setup` first with `models/{public_model}`; the gateway rewrites it, keeps the provider key server-side, and relays bidirectional audio, video, text, tool, and resumption messages under `realtime:connect`. |
...
| POST /api/anthropic/v1/messages | Anthropic message/content blocks, stop reasons, usage, typed content stream with a stable public model alias, bounded native JSON/SSE basic or dynamic `web_search_20250305`/`web_search_20260209`/`web_search_20260318` with `messages:web_search`, and `web_fetch_20250910`/`web_fetch_20260209`/`web_fetch_20260309`/`web_fetch_20260318` with `messages:web_fetch` |
| POST /api/anthropic/v1/messages/count_tokens | Target-appropriate counting; no invented exact cross-model token count |
| POST/GET /api/anthropic/v1/files | Create one encrypted key-owned PDF/text/image File (8 MiB max) or list 1–100 at a time with `next_page`; requires `files:manage`; current GA shape only |
| GET/DELETE /api/anthropic/v1/files/{file_id} | Retrieve metadata or delete a File owned by the creating key; a file ID in Messages/count_tokens document/image blocks expands before routing and counts against request limits |
| GET /api/anthropic/v1/files/{file_id}/content | Returns 400 for uploaded files, which Anthropic marks `downloadable:false`; generated provider files are unsupported |
...
[Anthropic Files](https://platform.claude.com/docs/en/build-with-claude/files) defines the GA file metadata, pagination, upload, and reference shapes. Gateway Files are local and key-owned; encrypted bytes are expanded into native inline content before inference instead of creating provider workspace files.
...
Gateway-owned OpenAI Vector Stores expose atomic create with optional same-key `file_ids`, list, retrieve, update, delete, backend text search, plus attach, list, retrieve, update, detach, parsed content, and atomic file batches for same-key gateway Files. They require `vector_stores:manage`, belong to the creating key, share retained-resource ceilings, support bounded metadata/attributes and optional 1–365-day expiry anchored at `last_active_at`, report active attachment bytes/counts, and use validated `after`/`before` keyset pagination. Create-with-files and file batches accept up to 2,000 unique IDs with auto chunking, roll back every resource if any input fails, and reuse the same key/expiry/capacity checks. File batches additionally accept shared or per-file attributes, complete in the creation transaction, and expose retrieval, idempotent cancellation, and paginated file listing through the pinned SDK. Parsed content accepts UTF-8/ASCII without NUL bytes, BOM-marked little- or big-endian UTF-16 with valid surrogate pairs, bounded `.html` visible text without head, script, style, template, or noscript content, a bounded `.docx` package containing one valid WordprocessingML main document, a bounded `.pptx` package with internal slide relationships, a bounded `.xlsx` package with internal worksheet and shared-string relationships, or a PDF with selectable text (at most 16 MiB and 256 pages). DOCX extraction includes body text, tabs, line breaks, paragraphs, and table text. PPTX extraction follows presentation order and includes DrawingML slide text, tabs, line breaks, and paragraphs. XLSX extraction follows workbook order and includes shared, inline, numeric, boolean, and cached formula values as tab-separated rows. PDF extraction is strict and sequential, limited to 10 seconds, 16 MiB per decoded stream and 16 MiB of extracted text, and bounded page work; image-only scans require OCR and fail explicitly. Content is derived into chunks of at most 64 KiB on request and duplicate plaintext is never stored. Search ranks those chunks by deterministic term-frequency cosine similarity, applies nested attribute filters on the server, supports explicit `ranker: none`, and rejects query rewriting or embedding-backed rankers. The same engine backs bounded same-key Responses `file_search`. Static token chunking, PDF OCR/scanned text, legacy Office parsing, notes, charts, embedded-media extraction, and embedding-backed semantic search remain unsupported.
...
Bounded native web search is a separate Responses capability. It requires one `web_search` tool at most, optional function tools, an OpenAI-adapter target using the built-in `openai` preset with both `chat` and `web_search` on the public and upstream model, `responses:web_search`, `max_tool_calls` from 1 through 4, and positive `max_output_tokens`. Direct input/instructions, metadata, function/tool-choice settings, text/reasoning settings, sampling, truncation, storage, and background fields stay native. Context size, live/cache access, approximate location, allowed- and blocked-domain filters, bounded `return_token_budget: "default"`, and `web_search_call.action.sources` are supported. The JSON output preserves native search/open/find actions, citations, sources, and token usage; request history and daily usage record completed search-call count separately. A configured price version can add a web-search fee per reported call to token cost. Spend policies require that fee, an explicit cache-read rate, and the existing bounded token estimates; free-only and lowest-cost search routing remain unsupported. Streaming, Conversations, previous-response/provider references, preview/image search, unlimited search-token return, translation, and post-dispatch fallback are unsupported. Stateless, default stored 30-day, and background modes all send `store:false` upstream. Provider-owned response chains, other hosted tools, files, and caches remain separate resource contracts.

Gateway-owned Responses file search uses one `file_search` tool with 1–10 same-key local Vector Store IDs, `responses:file_search` plus `responses:generate`, a native OpenAI or OpenAI-compatible Responses target, positive `max_output_tokens`, and at most four model-selected search calls. The gateway converts the tool to a private strict function, executes bounded backend lexical search, returns only matched text to the same model, then restores the public tool contract and exposes official `file_search_call` output in provider order. Local File IDs and attachment attributes are never sent upstream; replaying a gateway-generated `file_search_call` as new input is rejected. If the same turn contains a caller-owned function call, the gateway returns both calls without sending that unresolved function into another provider turn. `max_num_results`, attribute filters, score threshold, forced/allowed tool choice, and optional `file_search_call.results` are supported. `max_output_tokens` is one cumulative ceiling across all provider turns. Usage from every completed turn, including turns before a later failure, is aggregated into one priced accounting attempt; local search adds no provider fee. Streaming, Conversations, previous-response references, prompt templates/cache controls, combined web search, semantic/hybrid ranker selection, translation, synthesized file citations, and post-dispatch fallback are rejected. Stateless, locally stored, and durable background Responses are covered by the pinned OpenAI SDK.

Native Anthropic hosted web tools accept one basic or dynamic-filtering search/fetch tool in JSON or SSE Messages, and one search tool and one fetch tool may appear together. Search uses `web_search_20250305`, `web_search_20260209`, or `web_search_20260318`; fetch uses `web_fetch_20250910`, `web_fetch_20260209`, `web_fetch_20260309`, or `web_fetch_20260318`. Each requires `max_uses` from 1 through 4. Dynamic versions default to Anthropic-managed `code_execution_20260120`; `allowed_callers` may force direct invocation or allow supported code-execution callers, and optional `strict` must be boolean. `response_inclusion` (`full` or `excluded`) is accepted only on the 20260318 search/fetch versions, and fetch `use_cache` only on `web_fetch_20260309` or `web_fetch_20260318`. Search supports optional domain/location controls. Fetch also requires approximate text `max_content_tokens` from 1 through 16,384, which does not bound binary PDFs, and supports up to ten allowed or blocked plain hostnames plus optional citations. Public and upstream models need the matching base capability; code-execution invocation additionally needs `web_search_dynamic` or `web_fetch_dynamic`, `web_search_response_inclusion` or `web_fetch_response_inclusion` gates `response_inclusion`, and `web_fetch_cache_bypass` gates `use_cache`. Keys need `chat:generate` plus the matching `messages:web_*` scope, and a combined request needs both scopes and both capability sets. Targets use the built-in Anthropic preset. Native result/error/caller blocks, citations, terminal token/search/fetch/code-execution usage, and the public model alias are preserved. Prompt caching may be combined, including a `cache_control` breakpoint on the tool definition; cache counters and web-tool usage are preserved separately. Token/spend policies, lowest-cost routing, and free-only routing reject combined requests because fetch input and fees remain unbounded or unpriced even when a search-call fee is configured. Combined requests still reject translation, deferred loading, and post-dispatch fallback. Fetch also rejects local Message Batches.
...
`GET /api/v1/usage/breakdown` accepts `dimension=key|user|model|connection|dialect|operation|state|mode` and `sort=requests|known_cost|tokens|p95_latency|p95_first_byte`, plus the same time, user, key, model, and connection filters as usage summary. It returns request counts, attempt and failed-attempt counts, token/cache/search/tool counters, known spend, unknown attempts, nullable average/p95 whole-gateway request duration, and nullable average/p95 time to first byte per group. The `mode` dimension separates streaming requests (server-sent events and Realtime/Live sessions) from synchronous ones. Time to first byte runs from request start to the first response byte of the successful attempt, so it includes routing and any fallback; for a synchronous response it is close to the full duration. Asynchronous work (background Responses, batches, media jobs) has no first-byte time. Request counts and duration use request start time; attempt usage and cost use attempt start time, matching the summary. Request counts are distinct across retries. A request that falls back across providers can appear under multiple connections, so provider request totals are not additive. Failed attempts can occur even when the final request succeeds. Known spend excludes attempts without a resolved cost and is not a provider invoice. The endpoint uses offset pagination (default 20, maximum 100 rows per page; maximum offset 10,000), separate from request history's cursor pagination. Retained aggregate history can outlive inspectable request detail.
