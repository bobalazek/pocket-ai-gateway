...
Gateway-owned OpenAI Files are encrypted in the system store and belong to the creating inference key. Files share the retained-resource count and byte pools with stored Responses, stored Chat Completions, and Anthropic Message Batch items: at most 250 resources or 128 MiB per key, 2,500 resources or 512 MiB per owner, and 10,000 resources or 1 GiB per instance. Expired resources stop consuming pooled capacity and are removed by scheduled retention. Files expire after 30 days by default, may request an earlier expiry, and cannot be referenced by inference operations in this bounded slice.
