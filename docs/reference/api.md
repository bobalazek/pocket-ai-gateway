...
| POST /api/openai/v1/batches | Create a key-owned 24-hour Batch from a same-key File containing 1–4 non-streaming `/v1/responses` requests; requires `batches:manage` plus `responses:generate` |
...
Gateway-owned OpenAI Responses Batches use the four official methods: create, retrieve, list, and cancel. Creation accepts only `completion_window: "24h"`, `endpoint: "/v1/responses"`, optional bounded metadata/output-file expiry, and an unexpired same-key `purpose=batch` input File. Each File contains one to four JSONL envelopes with unique 1–64-byte UTF-8 `custom_id`, `method: "POST"`, the matching URL, one public model, and non-streaming inline input. The local worker rechecks the creating key's current grants and executes through ordinary Responses routing, admission, accounting, and pricing with upstream `store:false`. Success and failure lines are written to separate same-key `purpose=batch_output` Files; output order is unspecified, so callers correlate by `custom_id`. Nested background work, Conversations, provider-owned response/File references, hosted tools, and local File references are rejected. The 24-hour processing deadline expires unfinished items while preserving completed output. Batch metadata expires 30 days after creation. OpenAI's provider-owned 50% discount, separate rate pool, 50,000-request/200 MB ceilings, and unbounded output-token behavior do not apply.
