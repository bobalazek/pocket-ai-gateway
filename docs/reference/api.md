...
| POST /api/openai/v1/batches | Create a key-owned 24-hour Batch from a same-key File containing 1–4 non-streaming `/v1/responses`, `/v1/chat/completions`, `/v1/embeddings`, or `/v1/moderations` requests; requires `batches:manage` plus the matching endpoint scope |
...
Gateway-owned OpenAI Batches use the four official methods: create, retrieve, list, and cancel. Creation accepts only `completion_window: "24h"`, endpoint `/v1/responses`, `/v1/chat/completions`, `/v1/embeddings`, or `/v1/moderations`, optional bounded metadata/output-file expiry, and an unexpired same-key `purpose=batch` input File. It requires `batches:manage` plus `responses:generate`, `chat:generate`, `embeddings:generate`, or `moderations:classify` for the chosen endpoint. Each File contains one to four JSONL envelopes with unique 1–64-byte UTF-8 `custom_id`, `method: "POST"`, the matching URL, one public model, and non-streaming inline input. Chat requests require non-empty `messages`; Embeddings requests require a non-empty string, string array, token-ID array, or token-ID matrix and may use positive `dimensions`, `encoding_format` of `float` or `base64`, and string `user`. Multi-input embedding arrays contain at most 2,048 inputs. Moderation requests reuse the ordinary Moderations validation. The local worker rechecks the creating key's current grants and executes through ordinary endpoint routing, admission, accounting, and pricing. Responses and Chat disable upstream storage; Embeddings use the public model's fixed vector-space target without fallback; Moderations require a native capable target and never fall back after dispatch. Success and failure lines are written to separate same-key `purpose=batch_output` Files; output order is unspecified, so callers correlate by `custom_id`. Moderation output keeps the ordinary native taxonomy, while Batch aggregate usage is null because the response has no usage object. Nested background work, provider-owned references, hosted tools including Chat `web_search_options`, and local File references are rejected where applicable. The 24-hour processing deadline expires unfinished items while preserving completed output. Batch metadata expires 30 days after creation. OpenAI's provider-owned 50% discount, separate rate pool, 50,000-request/200 MB ceilings, and unbounded output-token behavior do not apply.
...
| --- | --- |
| OpenAI | Native Chat, Responses, bounded hosted web search, and embeddings |
