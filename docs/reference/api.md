...
Gateway-owned OpenAI Vector Stores expose create, list, retrieve, update, and delete plus attach, list, retrieve, update, and detach for same-key gateway Files. They require `vector_stores:manage`, belong to the creating key, share retained-resource ceilings, support bounded metadata/attributes and optional 1–365-day expiry anchored at `last_active_at`, report active attachment bytes/counts, and use validated `after`/`before` keyset pagination. Create-with-`file_ids`, static chunking, file batches, parsed content, semantic search, and Responses `file_search` remain unsupported.
