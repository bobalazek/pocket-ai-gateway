...
func modelVisibleInDialect(dialect string, model providers.PublicModel, target providers.Target) bool {
	...
	operations := map[string][]struct{ scope, operation string }{
		"openai":    {{"embeddings:generate", "embeddings"}, {"moderations:classify", "moderations"}, {"tokens:count", "responses/input_tokens"}, {"images:generate", "images/generations"}, {"audio:speech", "audio/speech"}, {"audio:transcribe", "audio/transcriptions"}},
