...
func (service *Service) repriceItems(ctx context.Context, queryer interface {
	...
	rows, err := queryer.QueryContext(ctx, `SELECT attempts.id, requests.owner_user_id, requests.key_id, attempts.model_id, attempts.connection_id,
		...
		WHERE attempts.connection_id = ? AND attempts.model_id = ? AND attempts.started_at >= ? AND attempts.started_at < ? AND attempts.input_tokens IS NOT NULL AND attempts.output_tokens IS NOT NULL AND attempts.usage_status != 'unknown' ORDER BY attempts.started_at, attempts.id LIMIT 10001`, connectionID, modelID, from, to)
	if err != nil {
		return nil, 0, err
	}
	type rawItem struct {
