...
func checkRetainedResourceCapacity(ctx context.Context, query responseQueryer, ownerID, keyID string, incomingCount, incomingBytes int64) error {
	...
	err := query.QueryRowContext(ctx, `SELECT COUNT(*),COALESCE(SUM(size),0),
		...
		FROM (
			...
			UNION ALL
			SELECT openai_vector_stores.owner_user_id,openai_vector_stores.key_id,length(openai_vector_store_file_batches.id) AS size
			...
			WHERE openai_vector_stores.expires_at IS NULL OR openai_vector_stores.expires_at>?
			UNION ALL
