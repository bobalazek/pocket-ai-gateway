...
func TestOpenAIBatchesMigrationEnforcesEncryptedItemsAndCascade(t *testing.T) {
	...
	var version int64
	if err = database.QueryRowContext(ctx, `SELECT MAX(version) FROM schema_migrations`).Scan(&version); err != nil || version < 25 {
...
func TestOpenAIModerationBatchMigrationPreservesV24Rows(t *testing.T) {
	ctx := context.Background()
	...
	if _, err = database.ExecContext(ctx, `INSERT INTO openai_batches(id,owner_user_id,key_id,input_file_id,endpoint,completion_window,model_id,status,metadata_json,output_expiry_seconds,request_total,created_at,in_progress_at,expires_at,retention_expires_at) VALUES('batch_upgrade','usr_upgrade','key_upgrade','file_input','/v1/embeddings','24h','model','in_progress','{}',3600,1,1,1,86400001,2592000001)`); err != nil {
		t.Fatal(err)
	}
	...
	if err = database.QueryRowContext(ctx, `SELECT b.endpoint,i.custom_id FROM openai_batches b JOIN openai_batch_items i ON i.batch_id=b.id WHERE b.id='batch_upgrade'`).Scan(&endpoint, &customID); err != nil || endpoint != "/v1/embeddings" || customID != "preserved" {
