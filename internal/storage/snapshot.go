...
func encryptedDataExists(ctx context.Context, database *sql.DB) (bool, error) {
	for _, item := range []struct {
		table string
		where string
	}{{"provider_credentials", "ciphertext IS NOT NULL"}, {"openai_files", "1"}, {"openai_batch_items", "request_ciphertext IS NOT NULL OR result_ciphertext IS NOT NULL"}, {"openai_upload_parts", "ciphertext IS NOT NULL"}} {
		var exists int
