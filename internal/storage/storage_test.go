...
func TestOpenCreatesVerifiedPreMigrationSnapshot(t *testing.T) {
	...
	if _, err := database.ExecContext(ctx, "DROP TABLE gemini_uploads; DROP TABLE gemini_files; DROP TABLE anthropic_files; DROP TABLE media_jobs; DROP TABLE provider_adapter_scripts; DROP TABLE openai_vector_store_files; DROP TABLE openai_vector_store_file_batches; DROP TABLE openai_vector_stores; DROP TABLE openai_upload_parts; DROP TABLE openai_uploads; DROP TABLE openai_batch_items; DROP TABLE openai_batches; DROP TABLE openai_files; DROP TABLE message_batch_items; DROP TABLE message_batches; DROP TABLE stored_chat_completion_metadata; DROP TABLE stored_chat_completions; DROP TABLE conversation_items; DROP TABLE conversations; DROP TABLE stored_responses; DROP TABLE backup_jobs; DROP TABLE operation_settings; DROP TABLE catalog_refresh_state; DROP TABLE catalog_candidates; DROP TABLE route_observations; DROP TABLE public_model_targets; DROP TABLE public_models; DROP TABLE upstream_models; DROP TABLE provider_credentials; DROP TABLE provider_connections; DROP TABLE event_outbox; DROP TABLE pricing_jobs; DROP TABLE cost_assessments; DROP TABLE usage_ledger; DROP TABLE concurrency_leases; DROP TABLE reservations; DROP TABLE attempts; DROP TABLE price_versions; DROP TABLE requests; DROP TABLE admission_clock; DROP TABLE quota_periods; DROP TABLE bucket_state; DROP TABLE limit_policies; DROP TABLE audit_events; DROP TABLE api_key_secrets; DROP TABLE api_keys; DROP TABLE login_throttles; DROP TABLE activation_tokens; DROP TABLE user_sessions; DROP TABLE setup_tokens; DROP TABLE users; DELETE FROM schema_migrations WHERE version >= 2"); err != nil {
		t.Fatal(err)
	}
	...
	if err := validateSnapshotFiles(context.Background(), filepath.Dir(manifests[0]), manifest); err != nil {
		t.Fatal(err)
	}
