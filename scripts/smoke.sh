...
address=""
for _ in {1..100}; do
  address=$(grep -Eo 'listen=127\.0\.0\.1:[0-9]+' "$scratch/server.log" 2>/dev/null | tail -n 1 | cut -d= -f2 || true)
  [[ -n "$address" ]] && break
...
[[ -n "$csrf" ]]
curl --silent --fail --cookie "$scratch/cookies" "$base/api/v1/usage" | grep -q '"known_cost_usd":"0"'
curl --silent --fail --cookie "$scratch/cookies" "$base/api/v1/admin/diagnostics" | grep -q '"backup_key_configured":true'
curl --silent --fail --cookie "$scratch/cookies" --header "Origin: $base" --header "X-CSRF-Token: $csrf" --request POST \
  "$base/api/v1/admin/backups" | grep -q '"state":"succeeded"'
...
curl --silent --fail --cookie "$scratch/cookies" --header "Content-Type: application/json" --header "Origin: $base" --header "X-CSRF-Token: $csrf" \
  ...
  "$base/api/v1/admin/policies" | grep -q '"metric":"requests"'
...
activation_code=$(printf '%s' "$user_response" | grep -Eo '"activation_code":"[^"]+"' | cut -d'"' -f4)
[[ -n "$activation_code" ]]
if grep -Fq -- "$activation_code" "$scratch/server.log"; then
...
key_id=$(printf '%s' "$key_response" | grep -Eo '"id":"key_[^"]+"' | head -n1 | cut -d'"' -f4)
key_secret=$(printf '%s' "$key_response" | grep -Eo '"secret":"[^"]+"' | cut -d'"' -f4)
[[ -n "$key_id" && -n "$key_secret" ]]
if grep -Fq -- "$key_secret" "$scratch/server.log"; then
...
curl --silent --fail --cookie "$scratch/cookies" --header "Origin: $base" --header "X-CSRF-Token: $csrf" --header 'If-Match: "1"' --request POST \
  "$base/api/v1/keys/$key_id/rotate" | grep -q '"secret":"pag_'
...
curl --silent --fail --cookie-jar "$scratch/login-cookies" --header "Content-Type: application/json" --header "Origin: $base" \
  --data '{"email":"owner@example.test","password":"correct-horse-battery"}' "$base/api/v1/auth/login" | grep -q '"role":"owner"'
curl --silent --fail --cookie "$scratch/login-cookies" "$base/api/v1/auth/sessions" | grep -q '"current":true'
asset=$(curl --silent "$base/_/" | grep -Eo '/_/_next/static/[^" ]+\.(css|js)' | head -n 1)
[[ -n "$asset" ]]
...
grep -q 'already in use' "$scratch/second.log"

kill -TERM "$server_pid"
wait "$server_pid"
server_pid=""
grep -q 'gateway stopped' "$scratch/server.log"
...
./pocket-ai-gateway version | grep -q .
