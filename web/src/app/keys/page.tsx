...
const scopes = ["chat:generate", "responses:generate", "embeddings:generate", "models:read", "tokens:count"];
  ...
  return (
    <AppShell active="API keys">
      <main id="main-content" className="content management-page">
		...
		{secret && <Card className="secret-card" role="status"><div><strong>Copy the {secret.label} secret now</strong><code>{secret.value}</code><small>{secret.keyID} · only its verifier is stored</small></div><Button variant="outline" onClick={() => navigator.clipboard.writeText(secret.value)}>Copy</Button></Card>}
