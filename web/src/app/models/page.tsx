...
const capabilities = ["chat", "embeddings", "moderations", "count_tokens", "images", "audio_speech"];
...
export default function ModelsPage() {
  ...
  return <AppShell active="Models"><main id="main-content" className="content management-page">
    ...
    <section className="section-block"><h2>{models.length} available models</h2><div className="resource-list">{models.map((item) => {
      const model = item as PublicModel; const configured = routes[item.id] ?? []; const embeddingModel = item.capabilities.includes("embeddings"); const opaqueMediaModel = item.capabilities.includes("images") || item.capabilities.includes("audio_speech");
