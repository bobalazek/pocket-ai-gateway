...
export function useModels() {
  ...
  async function saveRoute(event: FormEvent<HTMLFormElement>, model: PublicModel) { event.preventDefault(); const form = new FormData(event.currentTarget); const selectedIDs = new Set(form.getAll("target").map(String)); const selected = targets.filter((target) => selectedIDs.has(target.id)).map((target) => ({ upstream_model_id: target.id, priority: Number(form.get(`priority:${target.id}`)), weight: Number(form.get(`weight:${target.id}`)), enabled: true })); setBusy(true); try { await pocketAIGatewayAdmin.models.updateRoute(model, { strategy: String(form.get("strategy")) as RouteStrategy, free_only: form.get("free_only") === "on", targets: selected }); await load(); } catch (failure) { setError(message(failure, "Route could not be saved")); } finally { setBusy(false); } }
  ...
  return { manager, models, targets, selectedTarget, setSelectedTarget, selectedStrategies, selectStrategy, routes, previews, catalog, catalogState, error, busy, create, saveRoute, preview, saveCatalog, refreshCatalog, publishCapabilities };
}
