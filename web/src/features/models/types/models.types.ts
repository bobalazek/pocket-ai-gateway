...
export type RoutePlan = { model_id: string; strategy: RouteStrategy; free_only: boolean; selection_reason: string; selection_message: string; targets: { upstream_model_id: string; connection_id: string; upstream_id: string; adapter: string; capabilities: string[]; priority: number; weight: number; estimated_cost_usd?: string; latency_ms?: number; sample_count: number; circuit_open_until?: string }[]; rejected: { upstream_model_id: string; connection_id: string; reason: string; message: string }[] };
