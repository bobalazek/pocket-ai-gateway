...
export const providerCapabilities = ["chat", "embeddings", "moderations", "count_tokens", "images", "image_edit", "image_variation", "audio_speech", "audio_transcription", "audio_translation"] as const;
...
export function availableCapabilities(connection: ProviderConnection, presets: ProviderPreset[]) {
  ...
  if (connection.preset === "custom") return supported;
  const operations = presets.find((preset) => preset.id === connection.preset)?.operations ?? [];
  return supported.filter((capability) => capabilityOperations[capability].some((operation) => operations.includes(operation)));
}
