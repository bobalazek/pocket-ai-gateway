...
function GrantEditor({ user, allowedScopes, onSaved, onError }: { user: ManagedUser; allowedScopes: string[]; onSaved: () => Promise<void>; onError: (error: unknown) => void }) {
  ...
  return <details className="grant-editor"><summary>Edit inference grants</summary><form onSubmit={save}><fieldset className="scope-grid"><legend>Maximum operation scopes</legend>{allowedScopes.map((scope) => <label key={scope}><input type="checkbox" name="scopes" value={scope} defaultChecked={user.grants.scopes.includes(scope)} /> <span>{scope}</span></label>)}</fieldset><GrantFields user={user}/><Button type="submit">Save grants</Button></form></details>;
}
